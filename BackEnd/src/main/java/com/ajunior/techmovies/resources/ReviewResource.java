...
public class ReviewResource {
	...
	@GetMapping(value = "/{id}")
	public ResponseEntity<ReviewDTODetails> findById(@PathVariable Long id){
		ReviewDTODetails dto = reviewService.findById(id);
		return ResponseEntity.ok().body(dto);
	}
	
	@PostMapping
	public ResponseEntity<ReviewDTODetails> insert(@Valid @RequestBody ReviewDTODetails dto) {
